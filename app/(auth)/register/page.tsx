...
import {
  ...
  validateRequired, getPasswordStrength,
...
export default function RegisterPage() {
  ...
  function validate() {
    ...
    const matchErr   = validatePasswordMatch(fields.password, fields.confirmPassword)
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="text-center">
        <h1 className="text-2xl font-bold tracking-tight text-navy">Create your account</h1>
        <p className="mt-1 text-sm text-muted-foreground">Start studying smarter today</p>
      </div>
      ...
      <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-4">
        <div className="grid grid-cols-2 gap-3">
          <FormField id="firstName" label="First name" autoComplete="given-name" placeholder="Jane"
            value={fields.firstName} error={errors.firstName} onChange={(e) => set("firstName", e.target.value)} />
          <FormField id="lastName" label="Last name" autoComplete="family-name" placeholder="Doe"
            value={fields.lastName} error={errors.lastName} onChange={(e) => set("lastName", e.target.value)} />
        </div>

        <FormField id="email" label="Email address" type="email" autoComplete="email"
          placeholder="you@example.com" value={fields.email} error={errors.email}
          onChange={(e) => set("email", e.target.value)} />

        <div className="flex flex-col gap-1.5">
          <PasswordField id="password" autoComplete="new-password" placeholder="Min. 8 characters"
            value={fields.password} error={errors.password} onChange={(v) => set("password", v)} />
          {strength && (
            <div className="flex items-center gap-2">
              <div className="flex flex-1 gap-1">
                {[1, 2, 3].map((i) => (
                  <div key={i} className="h-1 flex-1 rounded-full transition-colors duration-300"
                    style={{ backgroundColor: i <= strength.level ? STRENGTH_COLOR[strength.level] : "var(--color-border)" }} />
                ))}
              </div>
        ...
        <div className="flex flex-col gap-1">
          <div className="flex items-start gap-2.5">
            <Checkbox id="terms" checked={terms} className="mt-0.5 cursor-pointer"
              onCheckedChange={(c) => { setTerms(c === true); setErrors((e) => ({ ...e, terms: "" })) }} />
        ...
        <Button type="submit" disabled={loading} className="mt-1 h-10 w-full cursor-pointer bg-teal text-white hover:bg-teal/90">
