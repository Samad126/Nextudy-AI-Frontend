      ...
      <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-6">
        ...
        <nav className="hidden items-center gap-1 md:flex">
          {NAV_LINKS.map(({ label, href }) => (
            <a key={label} href={href} className="cursor-pointer rounded-lg px-3 py-2 text-sm text-muted-foreground transition-colors hover:bg-muted hover:text-foreground">
              {label}
            </a>
          ))}
        </nav>
        ...
        <button className="cursor-pointer rounded-lg p-2 transition-colors hover:bg-muted md:hidden" onClick={() => setOpen((o) => !o)} aria-label="Toggle menu">
        ...
        <div className="flex flex-col gap-1 border-b border-border bg-background px-6 pb-4 md:hidden">
          ...
          <div className="mt-2 flex gap-2 border-t border-border pt-2">
            ...
            <Link href="/register" className="flex-1">
              <Button size="sm" className="w-full cursor-pointer bg-teal text-white hover:bg-teal/90">Get started</Button>
            </Link>
          </div>
        </div>
      )}
